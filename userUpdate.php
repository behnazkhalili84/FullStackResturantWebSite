...
$email = $first_name = $last_name = $phone = $address = $address2 = $city = $province = $postalcode = "";
$emailErr = $first_nameErr = $last_nameErr = $phoneErr = $addressErr = $address2Err = $cityErr = $provinceErr = $postalcodeErr = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
   ...
   if(empty($_POST["retype_password"])){
    ...
    if(empty($emailErr) && empty($first_nameErr) && empty($last_nameErr) && empty($phoneErr) && empty($addressErr) && empty($cityErr) && empty($provinceErr) && empty($postalcodeErr)){
        
        // Prepare an update statement
        $sql = "UPDATE users SET email=?, firstName=?, lastName=?, phone=?, address=?, address2=?, city=?, province=?, postalCode=? WHERE userID = ?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssssi", $param_email,$param_first_name, $param_last_name, $param_phone, $param_address, $param_address2, $param_city, $param_province, $param_postalcode,$param_userID);
            ...
            if(mysqli_stmt_execute($stmt)){
                // Redirect to login page header("location: user.php");
                header("location: userEdit.php");
            } else{
                echo "Oops! Something went wrong. Please try again later.".mysqli_sqlstate($link);
            }
        }
         // Close statement
