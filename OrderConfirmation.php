...
?>
    ...
    <hr>
    <h2 class ="form-title"> You order has been registered successfully</h2>
       
       <?php
            $query = "SELECT MAX(orderID) AS latest_order FROM orders";
            $result = $link->query($query);
        
            if ($result->num_rows > 0) {
                $row = $result->fetch_assoc();
                $latest_order_id = $row["latest_order"];
             // Fetch order details
             $sql = "SELECT o.orderID, o.totalPrice, o.userID, o.date, m.name, m.description, m.price, i.quantity 
             FROM orders o
             INNER JOIN orderitems i ON o.orderID = i.orderID
             INNER JOIN menu m ON m.foodID = i.foodID
             WHERE o.orderID = ?";
     $stmt = $link->prepare($sql);
     $stmt->bind_param("i", $latest_order_id); // Bind the actual order ID
     $stmt->execute();
     $result = $stmt->get_result();

     if ($result->num_rows > 0) {
        echo "<div class='msg-container'>";
        echo "<h4 style='color:red;margin:5px 0;'>Order Detail</h4>";
        echo "<hr class='underline'>";

        while ($row = $result->fetch_assoc()) {
            // Display item details
            echo "<p class='Order-detail'>Item Name: " . $row["name"] . "</p>";
            echo "<p class='Order-detail'>Item Price: $" . number_format($row["price"], 2) . "</p>";
            echo "<p class='Order-detail'>Item Quantity: " . $row["quantity"] . "</p>";
            echo "<br>";
            $totalPrice = $row["totalPrice"];
            $date = $row["date"];
        }

        // Display total price and order date
        echo "<p class='Order-detail'><strong>Total Price: $" . number_format($totalPrice, 2)  . "</strong></p>";
        echo "<p class='Order-detail'>Order Date: " . $date . "</p>";
        echo "</div>";
    } else {
